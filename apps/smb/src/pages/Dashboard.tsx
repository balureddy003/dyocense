// This file was corrupted during a previous refactor.
// To avoid confusion and ensure stability, re-export the fixed dashboard.
export { default } from './DashboardFixed'
/*
...
const ws = useWebSocket(
    { tenantId: tenantId ?? '', token: token ?? '', enabled: !!tenantId && !!token },
    {
...
const fetchData = async () => {
    ...
    setError(null);
    try {
        const [health, alertsData, signalsData, metricsData, goalsData, tasksData, recs] = await Promise.all([
            ...
            api.getTasks(tenantId, token, 'today'),
...
const handleAskCoach = () => navigate('/coach');
        ...
        <Container size="xl" py="md">
            <Stack gap="lg">
                ...
                <Card radius="md" withBorder>
                    <MantineText size="sm" c="dimmed">{buildNarrative()}</MantineText>
                </Card>
...
{/* Quick Actions for SMB workflows */ }
<Group gap="sm">
    <Button variant="light" leftSection={<IconBulb size={16} />} onClick={handleAskCoach}>Ask Coach</Button>
    <Button variant="gradient" leftSection={<IconTarget size={16} />} onClick={() => navigate('/goals')}>Set a goal</Button>
    <Button variant="default" leftSection={<IconNote size={16} />} onClick={() => navigate('/planner')}>Open Action Plan</Button>
...
{/* Two-column layout: Coach Insights | Goals & Tasks */ }
<Grid gutter="xl">
    {/* Left: Coach Insights (collapsible) */}
    <Grid.Col span={{ base: 12, md: 4 }} id="recommendations-section">
        <Group justify="space-between" align="center" mb="xs">
            <MantineText fw={600}>Coach Insights</MantineText>
            <ActionIcon variant="subtle" onClick={() => setShowCoach((s) => !s)}>
                        ...
                        <Card shadow="sm" radius="md" withBorder>
                            <Stack gap="xs">
                                ...
                                <MantineText c="dimmed" size="sm">
                                    Recommendations appear here when new insights are available.
                                </MantineText>
                                <Button variant="light" onClick={handleAskCoach} leftSection={<IconBulb size={16} />}>
                                    Ask Coach
                                </Button>
                            </Stack>
                        </Card>
                            ...
                            <Stack gap="xl">
                                {showGoalsTasks && (() => {
                                    ...
                                    return (
                                        ...
                                        />
                                                {
                                        showGoalsTasks && goals.length === 0 && !loading && (
                                })()}
                                {showGoalsTasks && goals.length === 0 && !loading && (
                                    <Card shadow="sm" radius="md" withBorder>
                                        <Stack gap="xs">
                                            ...
                                            <Button variant="gradient" onClick={() => navigate('/goals')} leftSection={<IconTarget size={16} />}>
                                                Set a goal
                                            </Button>
                                        </Stack>
                                        {showGoalsTasks && (() => {
                                        ...
                                        {showGoalsTasks && (() => {
                                            ...
                                            return (
                                                ...
                                                />
                                                {
                                                showGoalsTasks && tasks.length === 0 && !loading && (
                                })()}
                                        {showGoalsTasks && tasks.length === 0 && !loading && (
                                            <Card shadow="sm" radius="md" withBorder>
                                                <Stack gap="xs">
                                                    <MantineText fw={600}>No tasks</MantineText>
                                                    <MantineText c="dimmed" size="sm">
                                                        Create an action plan to turn insights into progress.
                                                    </MantineText>
                                                    <Button variant="light" onClick={() => navigate('/planner')} leftSection={<IconNote size={16} />}>
                                                        Open Action Plan
                                                    </Button>
                                                </Stack>
                                            </Card>
                                        )}
                                        {/* Evidence panel coming soon: recent activity, data sources, insights history */}
                                    </Grid.Col>
                    </Grid>
                </Stack>
                </Container>
