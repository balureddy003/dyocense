import { API_BASE_URL, DEFAULT_HEADERS, ApiOptions } from "./config";
...
export async function getRuns<T>(fallback: T): Promise<T> {
  return fetchJSON<T>("/v1/runs?limit=5", { fallback });
}
