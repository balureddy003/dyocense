...
class LLMSummary:
    ...
    @staticmethod
    def from_dict(payload: Mapping[str, Any]) -> "LLMSummary":
        return LLMSummary(
            ...
            completion_tokens=int(payload["completion_tokens"]) if payload.get("completion_tokens") is not None else None,
            prompt=str(payload.get("prompt")) if payload.get("prompt") is not None else None,
        )
...
class KernelRunResult:
    ...
    def from_dict(payload: Mapping[str, Any]) -> "KernelRunResult":
        return KernelRunResult(
            ...
            llm_summary=LLMSummary.from_dict(payload["llm_summary"]) if payload.get("llm_summary") else None,
            raw=dict(payload),
        )
