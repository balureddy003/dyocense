...
usage JSONB DEFAULT jsonb_build_object(
  ...
  CONSTRAINT valid_plan_tier CHECK (
    plan_tier IN (
      ...
      'smb_starter', 'smb_growth', 'enterprise'
    )
  )
);
...
CREATE INDEX idx_trust_facts_tenant ON trust.compliance_facts (tenant_id, recorded_at DESC);

-- =====================================================================
-- VERSIONING SCHEMA (Goal versions history)
-- =====================================================================

CREATE TABLE versioning.goal_versions (
  version_id TEXT PRIMARY KEY,
  tenant_id TEXT NOT NULL REFERENCES tenants.tenants(tenant_id) ON DELETE CASCADE,
  project_id TEXT NOT NULL REFERENCES tenants.projects(project_id) ON DELETE CASCADE,
  goal TEXT NOT NULL,
  ops JSONB NOT NULL,
  data_inputs JSONB,
  playbook_id TEXT,
  knowledge_snippets TEXT[] DEFAULT ARRAY[]::TEXT[],
  parent_version_id TEXT,
  label TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  goal_hash TEXT NOT NULL,
  retrieval JSONB DEFAULT '{}',
